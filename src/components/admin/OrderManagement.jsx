    ...
    <div className="flex gap-5">
      ...
      <div className="flex-1 rounded-2xl p-5" style={card}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold font-sora text-[14px]" style={{ color: "#0F172A" }}>Order Management</h3>
          <span className="text-[12px] font-sora" style={{ color: "#94A3B8" }}>{orders.filter((o) => o.is_active).length} active</span>
        </div>
          ...
          {filtered.map((order) => (
            <div key={order.id} onClick={() => setSelectedOrder(order)}
              ...
              style={selectedOrder?.id === order.id
                ...
                : { border: "1px solid #E2E8F0", background: "transparent", opacity: order.is_active ? 1 : 0.5 }}
              onMouseEnter={(e) => { if (selectedOrder?.id !== order.id) e.currentTarget.style.background = "#F8FAFC"; }}
              onMouseLeave={(e) => { if (selectedOrder?.id !== order.id) e.currentTarget.style.background = "transparent"; }}
            >
              <div>
                <p className="text-sm font-semibold font-sora" style={{ color: order.is_active ? "#0F172A" : "#94A3B8", textDecoration: order.is_active ? "none" : "line-through" }}>
                  {order.id}
                </p>
                <p className="text-xs font-sora" style={{ color: "#64748B" }}>{order.customer}</p>
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2 py-0.5 rounded-full border font-sora ${STATUS_STYLES[order.status] || "text-[#64748B]"}`}>{order.status}</span>
                ...
                {!order.is_active && <span className="text-xs font-sora" style={{ color: "#CBD5E1" }}>Deleted</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {selectedOrder && (
        <div className="w-72 rounded-2xl p-5 flex flex-col gap-4 self-start" style={card}>
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setSelectedOrder(null)} className="text-lg leading-none" style={{ color: "#94A3B8" }}>✕</button>
          </div>

          <div className="flex items-center justify-between">
            <p className="font-semibold font-sora text-sm" style={{ color: "#0F172A" }}>{selectedOrder.id}</p>
            <span className={`text-xs px-2 py-0.5 rounded-full border font-sora ${STATUS_STYLES[selectedOrder.status]}`}>{selectedOrder.status}</span>
          </div>
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              <div key={item.label}>
                ...
                <p className="text-sm font-sora" style={{ color: "#0F172A" }}>{item.value}</p>
              </div>
            ))}
          </div>

          {selectedOrder.is_active && (
            <button onClick={() => { setConfirmId(selectedOrder.id); setSelectedOrder(null); }}
